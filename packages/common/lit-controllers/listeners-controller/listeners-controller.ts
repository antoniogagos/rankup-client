...
export type TargetListenersPair<EventsMap = void> = [target: EventTarget, listeners: ListenersObject<EventsMap>];
	...
	#toggleListeners(listenersByTarget: TargetListenersPair<EventsMap>[], enable: boolean) {
		...
		for (const [node, listeners] of listenersByTarget) {
			if (node) {
				...
				console.error("Invalid listener target. Maybe it's not ready yet", {
					node,
					listeners,
				});
			}
