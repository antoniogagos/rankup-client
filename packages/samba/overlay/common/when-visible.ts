import { cancelablePromise, type CancelablePromise } from './cancelable-promise.js';

const requestIdleCallback =
	window.requestIdleCallback ||
	function requestIdleCallback(cb: (p: { didTimeout: boolean; timeRemaining: () => number }) => void) {
 ...
 */
export function whenVisible(element: HTMLElement, opts?: { rootMargin?: string; threshold?: number; timeout?: number }): CancelablePromise<void> {
...
function _whenVisibleUsingIntersectionObs({ element, rootMargin, threshold }: { element: HTMLElement; rootMargin?: string; threshold?: number }) {
...
function _whenVisibleUsingSize(el: HTMLElement, opts: { timeout: number }): CancelablePromise<void> {
	const { timeout = 2500 } = opts ?? {};
	let idleReq: number | null;
	let raf: number;
