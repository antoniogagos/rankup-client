import type { IEventsMap } from '../types/types.js';
...
// @ts-ignore
import BoxOverlayStyles from './styles/box-overlay.css';
import type { AnimationOptions, EventsMap, HorizontalAlign, Options, ReactiveControllerHostElement, Rect, VerticalAlign } from './types';
import { ListenersController } from '@rankup/common/lit-controllers/listeners-controller/listeners-controller';
 ...
 */
export class OverlayController<T extends ReactiveControllerHostElement = ReactiveControllerHostElement, EM extends IEventsMap = EventsMap<any>> implements ReactiveController {
	/**
	...
	async open() {
		...
		if (!this.host.isConnected) {
			const b4evtGlobal = this.#dispatchEvent('before-open-overlay', { ...evtDetail, wait: null }, { target: window });
			try {
	...
	async close(opts?: { noAnimation?: boolean }) {
		...
		this.#isClosing = true;
		try {
			this.#dispatchEvent('before-close-overlay', {
				overlayController: this,
				overlay: this.host,
			});
			this.endAnimation();
	...
	#attachToContainer() {
		if (!this.#attachedToContainer) {
			...
			const { host } = this;
			if (!container) {
				const evt = this.#dispatchEvent('find-overlay-container', { overlay: host, overlayController: this, container: null }, { target: window });
	...
	async #animateEntry(): Promise<void> {
		...
		return new Promise(resolve => {
			...
			if (this.withAnimation && this.animationIn != null) {
				const animationIn = Array.isArray(this.animationIn) ? this.animationIn : this.#getAnimationPreset(this.animationIn);
				if (animationIn) {
		...
		return new Promise(resolve => {
			try {
				if (this.withAnimation && this.animationOut != null) {
					const animationOut = Array.isArray(this.animationOut) ? this.animationOut : this.#getAnimationPreset(this.animationOut);
					if (animationOut) {
	...
	updatePosition() {
		...
		if (targetRect != null) {
			...
			const cssBottom = Number.isFinite(hostRectUpdated.bottom) ? Math.round(hostRectUpdated.bottom) + 'px' : 'auto';
			const cssLeft = positionizeLeft ? Math.round(hostRectUpdated.left) + 'px' : 'auto';
			const cssRight = Number.isFinite(hostRectUpdated.right) ? Math.round(hostRectUpdated.right) + 'px' : 'auto';
	...
	#getHorizontalPosition({ hostRect, targetRect }: { hostRect: Rect; targetRect: Rect }): {
		...
		const leftWidth = (this.noHorizontalOverlap ? targetRect.left : targetRect.right) - this.globalMargin;
		const rightWidth = windowWidth - this.globalMargin - (this.noHorizontalOverlap ? targetRect.right : targetRect.left);
		...
		const rightWidthFromCenter = windowWidth - this.globalMargin - targetRect.right + targetRect.width / 2;
		const fitsCenter = leftWidthFromCenter >= rectWidth / 2 && rightWidthFromCenter >= rectWidth / 2;
		/** @type {number | null} */
		let left = null;
		/** @type {number | null} */
		let right = null;
		...
		switch (this.horizontalAlign) {
			case 'left':
				if (fitsLeft) {
					right = windowWidth - (this.noHorizontalOverlap ? targetRect.left : targetRect.right) - offset;
				} else {
					left = 0;
	...
	#getVerticalPosition({ hostRect, targetRect }: { hostRect: Rect; targetRect: Rect }): {
		...
		const topHeight = (this.noVerticalOverlap ? targetRect.top : targetRect.bottom) - this.globalMargin;
		const bottomHeight = windowHeight - this.globalMargin - (this.noVerticalOverlap ? targetRect.bottom : targetRect.top);
		...
		const bottomHeightFromMiddle = windowHeight - this.globalMargin - targetRect.bottom + targetRect.height / 2;
		const fitsMiddle = topHeightFromMiddle >= rectHeight / 2 && bottomHeightFromMiddle >= rectHeight / 2;
		...
		switch (this.verticalAlign) {
			case 'top':
				if (fitsTop) {
					bottom = windowHeight - (this.noVerticalOverlap ? targetRect.top : targetRect.bottom) + offset;
				} else {
					top = offset;
					fitAtDesiredPosition = false;
				}
	...
	#dispatchEvent<EventName extends Events>(eventName: EventName, detail: EventsMap<OverlayController>[EventName]['detail'], opts?: { target?: EventTarget; cancelable?: boolean }): EventsMap<OverlayController>[EventName] | null {
			...
			if (this.positionTarget.ownerDocument !== document) {
				...
				if (ownerWindow) {
					...
					if (frameElement) {
						...
						const ownerWindowWidth = ownerWindow.visualViewport?.width ?? ownerWindow.document.body.offsetHeight;
	...
	#computeSlideInAnimation({ verticalAlign, horizontalAlign, noVerticalOverlap, isPositionedFromViewport }: { verticalAlign: VerticalAlign; horizontalAlign: HorizontalAlign; noVerticalOverlap: boolean; isPositionedFromViewport: boolean }) {
		if (noVerticalOverlap && verticalAlign !== 'middle') {
			if (verticalAlign === 'top') {
				return isPositionedFromViewport ? AnimationsPresets['slide-in-from-top'] : AnimationsPresets['slide-in-from-bottom'];
			}
			return isPositionedFromViewport ? AnimationsPresets['slide-in-from-bottom'] : AnimationsPresets['slide-in-from-top'];
		}
		if (horizontalAlign === 'left') {
			return isPositionedFromViewport ? AnimationsPresets['slide-in-from-left'] : AnimationsPresets['slide-in-from-right'];
		}
		return isPositionedFromViewport ? AnimationsPresets['slide-in-from-right'] : AnimationsPresets['slide-in-from-left'];
	}
}
