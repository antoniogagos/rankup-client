...
(function (RankingType) {
    RankingType[RankingType["SEASON"] = 1] = "SEASON";
    RankingType[RankingType["MATCHDAY"] = 2] = "MATCHDAY";
})(RankingType || (RankingType = {}));
/**
 * @fires selected-ranking-changed
 */
let RkTourneyRanking = class RkTourneyRanking extends LitElement {
    hidden = true;
    shouldUpdate() {
        return !this.hidden;
    }
    selectedRanking = RankingType.MATCHDAY;
    _ranking = new Task(this, () => rkApp.ds.GetRanking(), () => [null]);
    _onSelectRankingKeydown() {
        //
    }
    _onSelectRankingClick(evt) {
        const selectedRanking = evt.target.getAttribute('name');
        if (selectedRanking) {
            this.selectedRanking = Number(selectedRanking);
            this.dispatchEvent(new CustomEvent('selected-ranking-changed', {
                detail: { selectedRanking: Number(selectedRanking) },
            }));
        }
    }
    _renderRankingRow(user) {
...
], RkTourneyRanking.prototype, "hidden", void 0);
__decorate([
    property({ type: Number })
], RkTourneyRanking.prototype, "selectedRanking", void 0);
RkTourneyRanking = __decorate([
    customElement('rk-tourney-ranking')
], RkTourneyRanking);
export { RkTourneyRanking };
//# sourceMappingURL=rk-tourney-ranking.js.map