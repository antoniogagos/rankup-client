import type { Match, MatchStatus } from '../data-service/data-service.js';
...
//   DELAYED = 'DELAYED',
const matchHasNotStarted = new Set(['NS', 'POSTP', 'TBA', 'DELAYED']);
export function hasMatchdayStarted(matches: Match[]): boolean {
  const unstartedMatchday = matches.every(match =>
    matchHasNotStarted.has(match.status as MatchStatus),
  );
