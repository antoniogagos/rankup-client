...
import { AuthenticationDetails, CognitoIdToken, CognitoRefreshToken, CognitoUser, CognitoUserAttribute, CognitoUserPool } from './amazon-cognito-identity-js.js';
...
let _scheduledSessionRefresh: ReturnType<typeof setTimeout> | null = null;

export class SessionManager implements ReactiveController, ISessionManager {
	...
	private async _signupInWithCognitoUserPass(email: string, password: string, username: string): Promise<{ email: string; userId: string; userConfirmed: boolean }> {
		return new Promise((resolve, reject) => {
	...
	private _cognitoUserSessionToSession(session: CognitoUserSession, userConfirmationNecessary?: boolean): Session {
	...
	private dispatch<EventName extends keyof EventsMap>(eventName: EventName, detail: EventsMap[EventName]['detail'], opts?: { bubbles?: boolean; composed?: boolean }) {
