// import { IDisposable } from '@rankup/common/utils/disposable';
import { ManagerClass } from '@rankup/common/utils/managers.js';
import type {
...
import type { AppShell } from 'app-shell/app-shell.js';
...
interface AmazonCognitoIdentity {
	...
	AuthenticationDetails: typeof AuthenticationDetails;
	CognitoAccessToken: typeof CognitoAccessToken;
	CognitoIdToken: typeof CognitoIdToken;
	CognitoRefreshToken: typeof CognitoRefreshToken;
	CognitoUserAttribute: typeof CognitoUserAttribute;
	CognitoUserPool: typeof CognitoUserPool;
	CognitoUserSession: typeof CognitoUserSession;
	CookieStorage: typeof CookieStorage;
	WordArray: typeof WordArray;
	appendToCognitoUserAgent: typeof appendToCognitoUserAgent;
}
...
export class SessionManager extends ManagerClass<AppShell, EventsMap> {
	host: AppShell;

	Cognito?: AmazonCognitoIdentity;

	constructor(host: AppShell) {
		super(host);
		this.host = host;
	...
	hostConnected() {
		// set session immediately - we do this at next task to let previously registered consumers update their instances
		this.session = this._getSessionFromLocalStorage();
		// lazy-load aws cognito after the session is temporarily restored
		requestIdleCallback(() => this._initialize());
	}

	private _sessionInitialized = false;

	private _loadingSessionFirstTime = false;

	private async _initialize() {
		if (this._loadingSessionFirstTime || this._sessionInitialized) {
			return;
		}
		this._loadingSessionFirstTime = true;
		try {
			await import('amazon-cognito-identity-js');
			this.Cognito = window.AmazonCognitoIdentity;
			await this._finishExternalProviderLoginIfNeeded();
			this._restoreAndRefreshSession();
			this._sessionInitialized = true;
		} finally {
			this._loadingSessionFirstTime = false;
		}
	}
	...
	waitLoginComplete() {
		return new Promise<void>(resolve => {
			if (this._sessionInitialized) {
				...
				if (!this.session) {
					resolve();
				} else if (!this._sessionInitialized) {
					// _initialize is called lazily, but we might require it asap: (reloading protected page)
					this._initialize().then(() => resolve());
				}
			}
		});
	}

	private _getSessionFromLocalStorage(): Session | null {
		try {
	...
	signOut() {
		const { UserPoolId, ClientId } = this.host.env.Auth;
		const { CognitoUserPool } = this.Cognito!;
	...
	async forgotPassword(email: string): Promise<void> {
		const { UserPoolId, ClientId } = this.host.env.Auth;
		const { CognitoUser, CognitoUserPool } = this.Cognito!;
	...
	async changePassword(email: string, oldPassword: string, newPassword: string): Promise<void> {
		...
		return new Promise((resolve, reject) => {
			cognitoUser.changePassword(oldPassword, newPassword, (err?: Error, result?: 'SUCCESS') => {
				if (err) {
					console.error(err.message || JSON.stringify(err));
					reject(err);
				} else {
	...
	async resendConfirmationCode(email: string): Promise<ClientMetadata> {
		const { UserPoolId, ClientId } = this.host.env.Auth;
		const { CognitoUser, CognitoUserPool } = this.Cognito!;
		const cognitoUser = new CognitoUser({
			Username: email,
		...
		return new Promise((resolve, reject) => {
			cognitoUser.resendConfirmationCode((err?: Error, result?: ClientMetadata) => {
				if (err) {
					console.error(err.message || JSON.stringify(err));
					reject(err);
				} else {
	...
	async refreshSession(): Promise<Session | undefined> {
		...
		const { CognitoRefreshToken } = this.Cognito!;
		const cognitoUser = this._getCurrentCognitoUser();
		if (cognitoUser) {
			const sessionToken = new CognitoRefreshToken({ RefreshToken: this.session.refreshToken });
	...
	private async _finishExternalProviderLoginIfNeeded(): Promise<void> {
		const urlParams = new URLSearchParams(window.location.search);
		if (urlParams.has('googleAuth') && urlParams.has('code')) {
			const code = urlParams.get('code');
			if (code) {
				const { ClientId } = this.host.env.Auth;
	...
	private async _getSessionFromCode(code: string, provider: Providers): Promise<Session> {
		const { RedirectURI, ClientId, OAuthServerURL } = this.host.env.Auth;
		const { CognitoIdToken } = this.Cognito!;
	...
	private async _signupInWithCognitoUserPass(
		...
		return new Promise((resolve, reject) => {
			const { UserPoolId, ClientId } = this.host.env.Auth;
			const { CognitoUserAttribute, CognitoUserPool } = this.Cognito!;
	...
	private async _signInInWithCognitoUserPass(email: string, password: string): Promise<Session> {
		const { UserPoolId, ClientId } = this.host.env.Auth;
		const { AuthenticationDetails, CognitoUser, CognitoUserPool } = this.Cognito!;
	...
	private _scheduleRefreshSession(session: Session | null) {
		if (!this.Cognito) {
			// once loaded - it'll automatically check the session expiration and perform a refresh
			return;
		}
		if (_scheduledSessionRefresh != null) {
			clearTimeout(_scheduledSessionRefresh);
		}
		if (session) {
