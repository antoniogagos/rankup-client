# ADR 0043: Domain tourney umbrella + app-owned composition root
...
-   Create a single domain package: `packages/domain-tourney/`.
-   Model submodules **inside** the package (e.g., matchdays, submissions, ranking, ranked) rather than separate packages.
-   Contracts live in `packages/domain-tourney/src/contracts/**` and implementations in `packages/domain-tourney/src/browser/**`.
-   Mocks and fixtures live in `packages/domain-tourney/src/mock/**` with deterministic seeds.
...
-   [ ] Create `packages/domain-tourney` (contracts/browser/mock/common/tests).
-   [ ] Move `ITourneyService` contract + implementation into domain package.
