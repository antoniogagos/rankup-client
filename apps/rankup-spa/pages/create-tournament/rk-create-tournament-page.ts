...
@customElement('rk-create-tournament-page')
export class RkCreateTournamentPage extends LitElement {
	...
	private _onNameInput(event: Event) {
		const target = event.currentTarget as HTMLInputElement | null;
		this._tournamentName = target?.value ?? '';
		this._errorMessage = null;
	}
	...
	private async _onCreateTournament() {
		...
		const name = this._tournamentName.trim();
		if (!league || !name) {
			this._errorMessage = msg('Completa todos los campos para continuar.', { id: 'apps.rankup.spa.pages.create.tournament.rk.create.tournament.page.msg.l114c25' });
			return;
		}
		...
		try {
			...
			if (!service) {
				this._errorMessage = msg('Servicio no disponible. Inténtalo de nuevo.', { id: 'apps.rankup.spa.pages.create.tournament.rk.create.tournament.page.msg.l122c26' });
				return;
			}
			const request = this._buildRequest(league, name);
			await service.createTournament(request);
			window.dispatchEvent(new CustomEvent('rk-tournament-list-refresh'));
			this._navigateTo(path('TOURNAMENTS'));
		} catch (error) {
			console.error(error);
			this._errorMessage = msg('No se pudo crear el torneo. Inténtalo de nuevo.', { id: 'apps.rankup.spa.pages.create.tournament.rk.create.tournament.page.msg.l131c25' });
		} finally {
	...
	override render() {
		const canSubmit = Boolean(this._selectedCompetitionId && this._tournamentName.trim().length > 0);
		const buttonLabel = this._isSubmitting ? msg('Creando...', { id: 'apps.rankup.spa.pages.create.tournament.rk.create.tournament.page.msg.l139c44' }) : msg('Empezar liga', { id: 'apps.rankup.spa.pages.create.tournament.rk.create.tournament.page.msg.l139c64' });
		return html`
			<header>
				<button id="arrow" @click=${this._onClickGoBack} class="link--primary">${Icons('arrow-left', 20)}</button>
			</header>
			<main>
				<h1 class="text-bold">${msg('Crear una liga', { id: 'apps.rankup.spa.pages.create.tournament.rk.create.tournament.page.msg.l145c29' })}</h1>
				<div class="f3 mt-5 mb-3">${msg('Escoge una competición:', { id: 'apps.rankup.spa.pages.create.tournament.rk.create.tournament.page.msg.l146c33' })}</div>
				${leagueOptions.map(
					option => html`
				...
				<p class="f4 mt-5">${msg('Introduce el nombre de la liga.', { id: 'apps.rankup.spa.pages.create.tournament.rk.create.tournament.page.msg.l165c26' })}</p>
				<input
					class="mb-3"
					id="textInput"
					type="text"
					placeholder=${msg('Nombre de la liga', { id: 'apps.rankup.spa.pages.create.tournament.rk.create.tournament.page.msg.l170c20' })}
					.value=${this._tournamentName}
...
declare global {
	interface HTMLElementTagNameMap {
		'rk-create-tournament-page': RkCreateTournamentPage;
	}
}
