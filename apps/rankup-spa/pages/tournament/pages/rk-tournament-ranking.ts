 ...
 */
@customElement('rk-tournament-ranking')
export class RkTournamentRanking extends LitElement {
	@property({ type: Boolean }) override hidden = true;

	@service(ITournamentRankingService) private readonly _tournamentService!: ITournamentRankingService;
	...
	private _ranking = new Task(
		...
		() => {
			...
			return this._tournamentService.getTournamentRanking({ tournamentId });
		},
		...
		return html`
			<div class="main">
				${this.selectedRanking === RankingType.MATCHDAY
					? html`
						<div class="matchday-status text-bold">
							...
							${msg('Jornada en juego', { id: 'apps.rankup.spa.pages.tournament.pages.rk.tournament.ranking.msg.l105c10' })}
						</div>
					`
					: ''}
				<div class="ranking">
				...
				<div class="buttons mb-3" @click=${this._onSelectRankingClick} @keydown=${this._onSelectRankingKeydown}>
					<button class="btn btn--primary btn--s f5" name=${RankingType.SEASON} ?selected=${this.selectedRanking === RankingType.SEASON}>${msg('Temporada', { id: 'apps.rankup.spa.pages.tournament.pages.rk.tournament.ranking.msg.l116c135' })}</button>
					<button class="btn btn--primary btn--s f5 matchday-btn" name=${RankingType.MATCHDAY} ?selected=${this.selectedRanking === RankingType.MATCHDAY}>
						${msg('Jornada 8', { id: 'apps.rankup.spa.pages.tournament.pages.rk.tournament.ranking.msg.l118c9' })}
	...
	static override styles = [
		...
		/* row season winner + rk-tournament-header height */
...
declare global {
	interface HTMLElementTagNameMap {
		'rk-tournament-ranking': RkTournamentRanking;
	}
}
