...
export class RkTourneyMatchday extends LitElement {
	...
	@service(ITourneyMatchdaysService) private readonly _tourneyService!: ITourneyMatchdaysService;

	private _fixture = new Task(
		this,
		() => {
			const tournamentId = getCurrentTourneyId() ?? 'fj_rew';
			return this._tourneyService.getMatchdayMatches({ tournamentId, matchday: 1 });
		},
		() => [],
	);
				...
				complete: fixture =>
					hasMatchdayStarted(fixture.items)
						? html`
							...
							<rk-tourney-matchday-live .fixture=${fixture.items}></rk-tourney-matchday-live>
						`
						: html`
							<header>
								<button class="matchday-select f4 text-bold">Jornada 20 ${Icons('chevron-down', 16)}</button>
							</header>
							<rk-tourney-matchday-not-started .fixture=${fixture.items}></rk-tourney-matchday-not-started>
						`,
			})}
		`;
	}
