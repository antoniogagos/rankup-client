...
export class RkConfirmRegistrationPage extends LitElement {
	...
	private async _confirmRegistration(email: string, code: string) {
		try {
			await this._sessionManager.confirmRegistration(email, code);
			redirectToPublicPage('SIGNIN');
		} catch (error: unknown) {
	...
	private _processEmailAndCodeFromURL() {
		...
		if (this.email && this.code) {
			...
			redirectToPublicPage('SIGNUP');
		}
	}

	private async _resendCode() {
		try {
			await this._sessionManager.resendConfirmationCode(this.email!);
		} catch (error: unknown) {
