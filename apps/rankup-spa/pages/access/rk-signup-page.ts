...
import { redirectToPublicPage } from '../../lib/public-navigation.js';
...
export class RkSignUpPage extends LitElement {
	@property({ type: Boolean }) _showPassword = false;

	@service(ISessionManager) private readonly _sessionManager!: ISessionManager;

	@query('form') declare form: HTMLFormElement;

	@query('#username') declare usernameInput: HTMLInputElement;

	@query('#password') declare passwordInput: HTMLInputElement;

	@query('#repeatedPassword') declare passwordRepeatInput: HTMLInputElement;

	@query('#email') declare emailInput: HTMLInputElement;

	@query('#signInButton') declare signInButton: HTMLButtonElement;
	...
	private async _signUp(email: string, password: string, username: string) {
		try {
			...
			await this._sessionManager.signUpWithPassword({
				email,
				password,
				username,
			});
		...
		return html`
			...
			<form action="#" method="POST" @submit=${this._onFormSubmit} @input=${this._onFormInput}>
				...
				<section>
					<div class="input-wrapper">
						${Icons('email-open', 24)}
						<input class="form-control" id="email" name="email" placeholder="Email" type="email" autocomplete="email" required />
					</div>
				</section>
