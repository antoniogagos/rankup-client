...
export class RkResetPasswordPage extends LitElement {
	...
	@property({ type: String }) codeFromURL?: string | null;

	@property({ type: String }) emailFromURL?: string | null;

	@service(ISessionManager) private readonly _sessionManager!: ISessionManager;

	@query('#verificationCode') declare verificationCodeInput: HTMLInputElement;
	...
	@property({ type: Boolean }) override get hidden() {
		return this._hidden;
	}
	...
	private async _resetPassword(email: string, verificationCode: string, newPassword: string) {
		try {
			const resp = await this._sessionManager.confirmForgottenPassword(email, verificationCode, newPassword);
