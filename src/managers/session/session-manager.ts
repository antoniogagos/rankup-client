...
const { ClientId, OAuthServerURL, RedirectURI, UserPoolId } = env;

let _session: Session = null;
let _scheduledSessionRefresh: NodeJS.Timeout = null;
...
export class SessionManager implements ReactiveController {
  ...
  async confirmRegistration(email: string, confirmCode: string): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      cognitoUser.confirmRegistration(confirmCode, true, (err: Error, result: any) => {
        if (err) {
          console.error(err.message || JSON.stringify(err));
          reject(err);
        } else {
  ...
  async refreshSession(): Promise<Session> {
    // if (this._sessionNeedsRefresh(_session)) {
  ...
  private _scheduleRefreshSession(session: Session) {
