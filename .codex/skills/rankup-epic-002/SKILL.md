...
description: Epic 002 execution guide (service layering + DI skeleton). Use when implementing DI primitives, composition root, typed UI bridge (AppServices), tourney capability services (core/matchdays/ranking/etc), or import guardrails for layering.
metadata:
    owner: rankup
    epic: '002'
...
-   Migrate one vertical slice to a domain service (start with Tourneys/Home)
...
-   **WP-002-04**: Domain: tourney capability services contract + implementation + registration
...
-   `apps/rankup-spa/lib/composition-root.ts` (prefer `registerPlatformServices.ts` + `registerTourneyDomainServices.ts`)
...
-   Domain service -> **rankup-domain-tourney-service**
